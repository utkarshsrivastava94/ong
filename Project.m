...
phim_low = 0.05;
phim_high = 0.25;
phim = (phim_high - phim_low).*rand(20,1) + phim_low;
phif = 0.005;
gammaw = 0.45; %psi/ft
gammao = 0.35; %psi/ft
qw = 350; %ft3/day
...
for j =1:t
    ...
    for i=1:imax
        ...
        e_33(i,j) = -e_31(i,j);
        e_43(i,j) = -e_41(i,j);
        e_44(i,j) = - phim(i,1)/dt;
        e_34(i,j) = phim(i,1)/dt;
    end
    for i = 1:imax
        %LHS
        if i == imax
    ...
    for i=1:imax
        ...
        Rpom(i,j) = (phim(i,1) * swm(i,j)/dt) - (e_31(i,j) * (sigmaz/sigma) * gammao * (hwf(i,j) - hwm(i,j)));
        Rswm(i,j) = -(phim(i,1) * swm(i,j)/dt) - (sigma * chi * km * lambdawf(i,j) * ( (sigmaz/sigma) * gammaw * (hwf(i,j) - hwm(i,j)) - (pcwof(i,j) - pcwom(i,j))));
        if i == 1
...
ylabel('Saturation over time in matrix');
hold on;
plot(time, Som1);
hold on;
y1(1:t) = swrm;
